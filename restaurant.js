...
var fails = 0;
if(!localStorage.miscore){
  var punticos=0;
}else{
  var punticos=localStorage.miscore;
}
if(!localStorage.mifoto){
  var f=parseInt(Math.random()*20)+1;
  var foto="f"+f+".jpg";
  localStorage.mifoto=foto;
  foto=localStorage.mifoto;
}else{
  foto=localStorage.mifoto;
}
console.log(foto);
...
$(document).ready(function(){
  ...
  if(localStorage.miscore){
    $("#capturarelnombre").hide();
      $("#todito").fadeIn("slow");
       if(localStorage.miscore>=25){
          $(".todito").hide();
      
          var nom=localStorage.minombre;
      
          $("#ganador").html("Lo lograste "+nom+"<br>"+"<img src=img/"+foto+">").fadeIn("fast");;
    }else{
        $(".todito").show();
    }
  }
  $("#arrancar").on("click",function(event){
    var palabra=$("#minombre").val();
    if(palabra!=""){
      localStorage.minombre=palabra;
      $("#capturarelnombre").fadeOut("fast");
      $(".todito").css("display","block");
      $("#ganador").hide();
    }
  });
...
function fireRule(rule) {
  ...
  */
  ...
  if(win){
    ...
    punticos++;
    localStorage.miscore=punticos;
   // alert(punticos);
    if(localStorage.miscore>=26){
      $(".todito").hide();
      $("#capturarelnombre").hide();
      var nom=localStorage.minombre;
